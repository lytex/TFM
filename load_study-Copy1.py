...
num2 = df.query("F1_val!=0")[ ["F1_val", "l1", "l2", "no_global_wavelet", "no_local_wavelet"] + [f"global_level_list_{x}" for x in range(1, 7)] + [f"local_level_list_{x}" for x in range(1, 4)]]
...
# %%
def plot_f1_per_level(num2, key="l2_bin", var_name="global_level_list_1", drop_cols=("l1", "l2", "F1_val"), plot_sum=False):
# def plot_group_f1(num2, key="l2_bin", , drop_cols=("l1", "l2", "F1_val"), plot_sum=False):
    grouped_df = num2.groupby([key, "F1_val_bin"]).agg("sum").reset_index().drop(columns=list(drop_cols))
    grouped_df[[key, "F1_val_bin", var_name]]
    plt.figure(figsize=(12, 8))
...
#     fig_sum.savefig(f"plot/optuna/heatmap/per_level/l2_sum_global_level{x}.png")
#     fig_mean.savefig(f"plot/optuna/heatmap/per_level/l2_mean_global_level{x}.png")
#     fig_sum, fig_mean = plot_f1_per_level(num2, key="l1_bin", var_name=f"global_level_list_{x}", plot_sum=True) 
#     fig_sum.savefig(f"plot/optuna/heatmap/per_level/l1_sum_global_level{x}.png")
#     fig_mean.savefig(f"plot/optuna/heatmap/per_level/l1_mean_global_level{x}.png")
# for x in range(1, 4):
#     fig_sum, fig_mean = plot_f1_per_level(num2, key="l2_bin", var_name=f"local_level_list_{x}", plot_sum=True) 
#     fig_sum.savefig(f"plot/optuna/heatmap/per_level/l2_sum_local_level{x}.png")
#     fig_mean.savefig(f"plot/optuna/heatmap/per_level/l2_mean_local_level{x}.png")
#     fig_sum, fig_mean = plot_f1_per_level(num2, key="l1_bin", var_name=f"local_level_list_{x}", plot_sum=True) 
#     fig_sum.savefig(f"plot/optuna/heatmap/per_level/l1_sum_local_level{x}.png")
#     fig_mean.savefig(f"plot/optuna/heatmap/per_level/l1_mean_local_level{x}.png")
